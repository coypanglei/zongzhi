 ...
 */
@ActivityScope
public class SginStaffListModel extends BaseModel implements SignStaffListContract.Model {
    @Inject
    Gson mGson;
    @Inject
    ...
    @Inject
    public SginStaffListModel(IRepositoryManager repositoryManager) {
    ...
    @Override
    public Observable<BaseBean<AddressBookListBean>> getSignStaffList(String departmentId) {
        Map<String, Object> map = new HashMap<>(8);
        map.put("Id", departmentId);
        return mRepositoryManager
                .obtainRetrofitService(AddressLookService.class)
                .getEmployee(SignUtil.paramSign(map));
    }
}