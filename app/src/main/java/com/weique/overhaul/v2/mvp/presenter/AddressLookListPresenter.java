 ...
 */
@ActivityScope
public class AddressLookListPresenter extends ReworkBasePresenter<AddressLookListContract.Model, AddressLookListContract.View> {
    @Inject
    ...
    @Inject
    public AddressLookListPresenter(AddressLookListContract.Model model, AddressLookListContract.View rootView) {
...
public void getAddressBookListDataByVideo(boolean pullToRefresh, boolean isLoadMore, String DepartmentId,String name) {
        try {
            handlePaging(pullToRefresh, isLoadMore);
            commonGetData(mModel.getAddressBookListData(pageSize,ignoreNumber, DepartmentId,name), mErrorHandler, itemBean -> {
                mRootView.setAddressBookListData(itemBean,isLoadMore);
