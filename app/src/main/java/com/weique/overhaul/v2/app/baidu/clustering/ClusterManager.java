...
package com.weique.overhaul.v2.app.baidu.clustering;
...
import com.weique.overhaul.v2.app.baidu.MarkerManager;
import com.weique.overhaul.v2.app.baidu.clustering.algo.Algorithm;
import com.weique.overhaul.v2.app.baidu.clustering.algo.NonHierarchicalDistanceBasedAlgorithm;
import com.weique.overhaul.v2.app.baidu.clustering.algo.PreCachingAlgorithmDecorator;
import com.weique.overhaul.v2.app.baidu.clustering.view.ClusterRenderer;
import com.weique.overhaul.v2.app.baidu.clustering.view.DefaultClusterRenderer;
...
public class ClusterManager<T extends ClusterItem> implements
    ...
    private MapStatusListenter mapStatusListenterl;

    public MapStatusListenter getMapStatusListenterl() {
        return mapStatusListenterl;
    }

    public void setMapStatusListenterl(MapStatusListenter mapStatusListenterl) {
        this.mapStatusListenterl = mapStatusListenterl;
    }

    public interface MapStatusListenter {
        void createZoomData(float zoom, LatLngBounds bound,boolean finish);
    }
    ...
    public void addList(Collection<T> items) {
        mAlgorithmLock.writeLock().lock();
        try {
            mAlgorithm.getItems().clear();
            mAlgorithm.addItems(items);
        } catch (Exception e) {
            Timber.e(e);
        } finally {
    ...
    public void onMapStatusChange(MapStatus mapStatus) {
        ...
        try {
            //缩放
            if (Math.abs(this.zoom - zoom) > 0.000001) {
                ...
                if (ObjectUtils.isNotEmpty(mapStatusListenterl)) {
                    mapStatusListenterl.createZoomData(zoom, mapStatus.bound,false);
                }
                //平移
            } else {
                if (mRenderer instanceof BaiduMap.OnMapStatusChangeListener) {
    ...
    public void onMapStatusChangeFinish(MapStatus mapStatus) {
        ...
        if (ObjectUtils.isNotEmpty(mapStatusListenterl)) {
            mapStatusListenterl.createZoomData(zoom, mapStatus.bound,true);
        }
     ...
     * @param mapStatus
     */
    public void updateMapPoints(MapStatus mapStatus) {
        if (mRenderer instanceof BaiduMap.OnMapStatusChangeListener) {
            ((BaiduMap.OnMapStatusChangeListener) mRenderer).onMapStatusChange(mapStatus);
        }
        mPreviousCameraPosition = mMap.getMapStatus();
